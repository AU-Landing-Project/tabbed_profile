...
if ((!$profiles || !is_array($profiles)) && $page_owner->canEdit()) {
	$profiles = array();
	$profiles[] = Profile::generateDefaultProfile($page_owner);
}
...
if (!$defaultdetected && elgg_get_plugin_setting('private_user_profile', PLUGIN_ID) == 'no') {
...
if ($page_owner->canEdit() && count($profiles) < TABBED_PROFILE_MAX_TABS) {
	...
	$tabs[] = array(
		...
		'href' => elgg_get_site_url() . 'ajax/view/tabbed_profile/edit?guid=' . $page_owner->guid,
